...
**Length.** To respect AJA's length guidance, the prospective program components (external-validation and clinical-extension protocols) and the methods-detail tables have been moved to Supplementary Materials, and the abstract is within 250 words. The main text remains somewhat above the 20-page research-article suggestion because the study is a *prespecified* survey-weighted analysis whose value rests on reporting its planned sensitivity analyses (worse-ear, high-frequency, tympanometry-based conductive exclusion, and confounder-vs-mediator treatments) transparently rather than selectively; we would gladly shorten further at the editor's discretion.
