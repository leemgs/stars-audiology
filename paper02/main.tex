...
\begin{abstract}
\noindent\textbf{Objective:} Large language models (LLMs) can extract structured audiology fields from clinical text, but a probabilistic model must never suppress referral for a time-critical emergency such as sudden sensorineural hearing loss (SSNHL). We specify and \emph{evaluate} a deterministic, guideline-derived red-flag layer that overrides model output, and release an open benchmark and reproducible harness. \textbf{Materials and Methods:} We derived seven red-flag rules from the SSNHL and tinnitus guidelines and evaluated them at two levels on an open benchmark of 71 expert-authored/synthetic cases (34 structured, 37 free-text; no patient data): \emph{rule coverage} on correct features, and \emph{end-to-end}, running free text through two open rule-based extractors (naive, improved) \emph{and an actual open medical LLM} (\texttt{google/medgemma-4b-it}), all scored through one harness. We report red-flag recall with 95\% Wilson CIs, specificity, over-referral, and per-field metrics. \textbf{Results:} On correct features the layer achieved 100\% sensitivity (19/19) and 100\% specificity (15/15). End-to-end recall was dominated by extraction and spanned three independent extractors on identical cases: naive 56\% (specificity 89\%), \textbf{MedGemma-4b-it 78\%} (14/18; specificity 100\%), and the tuned rule extractor 100\% (specificity 100\%); decoding was deterministic and reproducible, and MedGemma's misses concentrated in colloquial and ambiguous phrasing. \textbf{Discussion:} The deterministic layer guarantees a low model score never suppresses referral and, given correct features, misses no red flag; but end-to-end safety is bounded by extraction. Crucially, MedGemma is independent of the benchmark's authorship and cue lists, so its 78\% is an \emph{extractor-independent} measurement---evidence the benchmark is neither saturated nor gamed and that the tuned 100\% reflects benchmark-specific tuning. \textbf{Conclusion:} A finite-set 100\% is a target, not a deployment guarantee, and a real medical LLM misses about one urgent case in five here---so clinician verification is load-bearing. We release the benchmark, rules, extractors, metrics, and a model-independent harness that reproduces the MedGemma result on a free GPU.
\end{abstract}
...
\bibitem{agrawal2022llmextract} Agrawal M, Hegselmann S, Lang H, Kim Y, Sontag D. Large language models are few-shot clinical information extractors. In: \textit{Proceedings of the 2022 Conference on Empirical Methods in Natural Language Processing (EMNLP)}. Association for Computational Linguistics; 2022:1998--2022.
\bibitem{uzuner2010i2b2} Uzuner \"{O}, South BR, Shen S, DuVall SL. 2010 i2b2/VA challenge on concepts, assertions, and relations in clinical text. \textit{J Am Med Inform Assoc}. 2011;18(5):552--556.
\bibitem{singhal2023llmclinical} Singhal K, Azizi S, Tu T, et al. Large language models encode clinical knowledge. \textit{Nature}. 2023;620(7972):172--180.
...
\bibitem{wong2021sepsis} Wong A, Otles E, Donnelly JP, et al. External validation of a widely implemented proprietary sepsis prediction model in hospitalized patients. \textit{JAMA Intern Med}. 2021;181(8):1065--1070.
\bibitem{finlayson2021datashift} Finlayson SG, Subbaswamy A, Singh K, et al. The clinician and dataset shift in artificial intelligence. \textit{N Engl J Med}. 2021;385(3):283--286.
